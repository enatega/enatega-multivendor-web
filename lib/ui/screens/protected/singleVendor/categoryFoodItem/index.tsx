...
import { GET_CATEGORIES_SUB_CATEGORIES_LIST, GET_SUB_CATEGORIES } from "@/lib/api/graphql";
...
export default function CategoryItemScreen() {
  ...
  const [subCategoriesForMobile, setSubCategoriesForMobile] = useState<SubCategoryMobile[]>([]);
  ...
  const processedCategories: ProcessedCategory[] = categories?.map((category: any) => {
    const subCats = subcategoriesData?.subCategories?.filter(
  ...
  const selectedCategoryData = processedCategories?.find(category => category._id === categoryId);

  // Menu items for the sidebar
  const menuItems = categoriesSubCategoriesList?.fetchCategoryDetailsByStoreId?.map((item: any) => ({
    id: item.id,
    label: item.label,
  ...
  useEffect(() => {
    if (categoryId && processedCategories?.length) {
      const category = processedCategories.find(cat => cat._id === categoryId);
      if (category) {
  ...
  }, [categoryId, processedCategories]);
  ...
  function parentItemRenderer(item: MenuItem) {
    ...
    return (
      <div
        ...
        onClick={() => router.push(`/singleVendor/categoryItemScreen/${item.id}/${restaurantId}`)}
      >
        <span className={`mx-2 ${item.items && "font-semibold"} text-${isClicked ? "[#5AC12F]" : "gray-600"}`}>
          {item.label}
        </span>
      </div>
    );
  ...
  function itemRenderer(item: MenuItem) {
    ...
    return (
      <div
        ...
        onClick={() => handleScrollToSubcategory(toSlug(item.label as string), 120)}
      >
        <span className={`mx-2 ${item.items && "font-semibold"} text-${isClicked ? "[#5AC12F]" : "gray-600"}`}>
          {item.label}
        </span>
      ...
      <PaddingContainer>
        <div className="flex flex-col items-center gap-6 py-10">
          <div className="text-center py-6 text-gray-500 flex flex-col items-center justify-center">
            <EmptySearch />
            <p className="text-lg mt-4">Category not found or no items available</p>
          </div>
          <Button 
            label="Go Back" 
                ...
                <ul className="flex space-x-4 items-center w-max flex-nowrap">
                  {subCategoriesForMobile.map((subCategory, index) => (
                    <li key={index} className="shrink-0">
                      <button
                        className={`bg-${selectedSubCategory === subCategory.slug
                          ? "[#F3FFEE]"
                          : "gray-100"
                        } text-${selectedSubCategory === subCategory.slug
                          ? "[#5AC12F]"
                          : "gray-600"
                        } rounded-full px-3 py-2 text-[10px] sm:text-sm md:text-base font-medium whitespace-nowrap`}
                        onClick={() => handleScrollToSubcategory(subCategory.slug, 150)}
                      >
                        {subCategory.label}
                      </button>
                    </li>
      ...
      <PaddingContainer>
        <div className="flex flex-col md:flex-row w-full">
          ...
          <div className="w-full md:w-4/5 p-3 h-full">
            {selectedCategoryData.subCategories.map((subCategory, subCatIndex) => (
              <div
                key={subCatIndex}
                className="mb-8"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {subCategory.foods.map((food, foodIndex) => {
                    ...
                    const transformedFood = {
                      ...
                      image: food.image,
                      cuisines: [selectedCategoryData.title, subCategory.title].filter(Boolean),
