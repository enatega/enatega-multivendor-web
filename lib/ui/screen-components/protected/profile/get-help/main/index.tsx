...
export default function GetHelpMain() {
  ...
  const handleSendMessage = useDebounceFunction(() => {
    ...
    const finalTicketTitle = reason === "order related" 
      ? `Order Issue - ${orderId}` 
      : ticketTitle;
      
    // Include order ID in description if order-related
    const ticketDescription = reason === "order related"
      ? `Order ID: ${orderId}\n\n${description}`
      : description;
  ...
  const faqItems = [
    {
      header: "General order questions",
      content: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nullam quis risus eget urna mollis ornare vel eu leo. Cum sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Nullam id dolor id nibh ultricies vehicula ut id elit."
    },
    {
        ...
        <Accordion className="w-full">
          {faqItems.map((item, index) => (
            <AccordionTab 
              key={index} 
              header={
                <span className="text-base md:text-lg font-medium">{item.header}</span>
              }
            >
              <div className="p-3">
      ...
      >
        <div className="flex flex-col md:h-[500px] h-[450px]">
          ...
          <div className="flex-1 p-4 overflow-y-auto">
            <div className="mb-6">
              <h2 className="text-lg font-semibold mb-2">Hi, {userName}</h2>
          ...
          <div className="p-4 border-t border-gray-200 bg-white">
            <button
              ...
              className={`bg-[#5AC12F] text-white w-full py-3 rounded-full flex items-center justify-center ${
                ...
                (reason === "others" && !ticketTitle.trim()) ||
                !description.trim() || 
                isSubmitting ? 'opacity-70 cursor-not-allowed' : 'hover:bg-[#4CAF27]'
              }`}
            >
              {isSubmitting ? (
