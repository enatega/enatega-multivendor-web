...
function TrackingStatusCard({ orderTrackingDetails }: TrackingStatusCardProps) {
  ...
  const getStepStatus = (stepIndex: number) => {
    const STATUS_ORDER = ["PENDING", "ACCEPTED", "ASSIGNED", "PICKED", "DELIVERED"];
  ...
  const getStatusMessage = () => {
    ...
    switch (status) {
      ...
      case "ASSIGNED":
        if (orderTrackingDetails.assignedAt) {
          const assignedTime = new Date(orderTrackingDetails.assignedAt);
          const timeElapsed = Math.floor((now.getTime() - assignedTime.getTime()) / 60000);
      ...
      case "DELIVERED":
        if (orderTrackingDetails.deliveredAt) {
          const deliveredTime = new Date(orderTrackingDetails.deliveredAt);
          const deliveredString = deliveredTime.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'});
    ...
    <div className="bg-white rounded-lg shadow-sm p-4 w-full max-w-2xl">
      ...
      <div className="flex items-center mb-3">
        ...
        <div className="mr-3">
          ...
          {(orderTrackingDetails.orderStatus === "DELIVERED" || orderTrackingDetails.orderStatus === "COMPLETED") && 
            <div className="w-8 h-8 flex items-center justify-center bg-green-100 rounded-full">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-green-500" viewBox="0 0 20 20" fill="currentColor">
