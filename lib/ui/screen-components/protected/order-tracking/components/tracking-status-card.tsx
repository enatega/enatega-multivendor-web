...
function TrackingStatusCard({ orderTrackingDetails }: TrackingStatusCardProps) {
  ...
  const getEstimatedDeliveryTime = () => {
    ...
    const expectedTime = orderTrackingDetails.expectedTime ? new Date(orderTrackingDetails.expectedTime) : null;
    const now = new Date();
    ...
    if (expectedTime) {
      const remainingMinutes = Math.max(0, Math.ceil((expectedTime.getTime() - now.getTime()) / 60000));
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm p-4 w-full max-w-2xl">
      <div className="flex justify-between items-center mb-3">
        <h3 className="text-sm sm:text-base font-semibold">Estimated Delivery Time</h3>
      ...
      <div className="grid grid-cols-4 gap-2 mb-4">
        {[0, 1, 2, 3].map((index) => {
          ...
          return (
            <div key={index} className="h-2 rounded-full bg-gray-200 overflow-hidden">
              <div 
